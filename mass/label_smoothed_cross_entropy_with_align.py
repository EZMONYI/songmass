    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        if not ("word_ids" in sample.keys() and "attns" in net_output[1].keys()):
            ...
            for attn in attns:
                ...
                attn_loss_layer = torch.sum(attn_loss_layer, dim=-1, keepdim=True)
        ...
        # Log each loss separately to TensorBoard
        global_step = model.get_num_updates()
        self.writer.add_scalar('NLL/train', nll_loss.item(), global_step)
        self.writer.add_scalar('Smooth/train', smooth_loss.item(), global_step)
        self.writer.add_scalar('Attn/train', attn_loss.item(), global_step)
        loss = (
            (1.0 - self.eps) * nll_loss
            + eps_i * smooth_loss
            + self.attn_loss_weight * attn_loss
        )
        self.writer.add_scalar('Total_Loss/train', loss.item(), global_step)
